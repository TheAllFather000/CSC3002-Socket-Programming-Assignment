...
import socket, os, threading, hashlib, sys, json, time, statistics
...
if __name__ == "__main__":
    ...
    while True:
        ...
        # Send to users who have joined a specific group
            ...
            payload = json.dumps({"type": "group", "sender": username,
                                  "group": group, "body": " ".join(msg)})
            t0 = time.perf_counter()
            client.send(payload.encode())
            latency_ms = (time.perf_counter() - t0) * 1000
            tcp_latencies.append(latency_ms)
        ...
        # Send to every connected user (concurrency/scalability testing)
            ...
            payload = json.dumps({"type": "broadcast", "sender": username, "body": " ".join(msg)})
            t0 = time.perf_counter()
            client.send(payload.encode())
            latency_ms = (time.perf_counter() - t0) * 1000
            tcp_latencies.append(latency_ms)
            ...
            print("Commands: login | create_account | private | group | join | leave | broadcast | file | log_out")
