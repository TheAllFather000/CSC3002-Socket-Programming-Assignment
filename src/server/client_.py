...
import socket, re, os, threading, hashlib, sys, json, time, subprocess
import colorist
...
def file_sender():
    ...
    sock.bind(("0.0.0.0", file_listen_port))
    sock.listen()
    ...
    while True:
        try:
            ...
            path = Path(output_path + "/" + data.get("file"))
            if path.exists():
                if path.is_file():
                    file = open(output_path + "/" + data.get("file"), "rb")
                    line = file.read(1024)
                    while line:
                        conn.send(line)
                        line = file.read(1024)
            ...
            colorist.red("Exception: " + e.__str__())
            continue
...
def receive_file(peer_sock, peer_addr):
    ...
    try:
        ...
        if md5_hash.hexdigest() == expected_md5:
            safe_print(
                coloured(f"[File received] Saved as '{save_path}' — MD5 verified.")
            )
...
def receive():
    ...
    global logged_in
    global username, password, output_path
    while True:
        try:
            ...
            try:
                data = json.loads(message)
                msg_type = data.get("process")
                print(msg_type)
                if msg_type == "login" and data.get("status") == "success":
                    if not logged_in:
                        logged_in = True
                    ...
                    safe_print(colorist.red("[SYSTEM]"))
                elif msg_type == "login_fail":
                    safe_print(colorist.red("[Auth] Login failed."))
                elif msg_type == "file":
                    client_ips = re.split(
                        r"[;,\s]+", data.get("ip").replace("[", "").replace("]", "")
                    )

                    threading.Thread(
                        target=file_transfer, args=(data.get("file"), client_ips)
                    ).start()
                elif msg_type == "account_created":
                    safe_print(coloured("[Auth] Account created successfully."))

                elif msg_type == "private":
                    print("s")
                    safe_print(coloured(f"[{data['username']}]: {data['message']}"))

                elif msg_type == "system":
                    safe_print(coloured(f"[SYSTEM]: {data['body']}"))
...
def file_transfer(filename: str, ip: list):
    start = time.time()
    sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    sock.bind(("0.0.0.0", file_listen_port))
    sock.listen(len(ip))
    ...
    for i in ip:
        try:
            sock.connect((ip, file_listen_port))
            sock.send(json.dumps({"file": filename, "username": username}).encode())

            with open(output_path + "/" + filename, "wb") as fil:
                data = sock.recv(1024)
                if not data:
                    break
                while data:
                ...
                colorist.green(
                    f"[File] File '{filename}' obtained successfully. Placed in '{output_path}'"
                )

        except Exception:
            continue
    colorist.green(f"[File] Failed to obtain file '{filename}'.")


#  UDP presence ping
def presence_ping():
    global udp_port, ping_host
    udp_sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    print(udp_port, ping_host)
    # udp_sock.connect((ping_host, udp_port))
    while True:
        payload = json.dumps({"command": "ping", "username": username})
        udp_sock.sendto(payload.encode(), (ping_host, udp_port))
        time.sleep(5)
...
def pick_colour():
    ...
    for k, (colour_code, ansi_code, name) in COLOURS.items():
        ...
        print(
            _c("  │  ", BLUE)
            + f"  {_c(k, CYAN, BOLD)})  {swatch}  {ansi_code}{name}{RESET}"
        )
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if user_input.startswith("login"):
        ...
        elif user_input.startswith("get_file"):
            file = input(_c("  Filepath: ", BLUE))
            if not file:
                colorist.red("Please supply a valid value for field 'file'")
                continue
            payload = json.dumps(
                {"username": username, "file": file, "command": "file"}
            )
            client.send(payload.encode())
        elif user_input.startswith("create_group"):
            group = input(_c("  Group name: ", BLUE))
            payload = json.dumps(
                {"command": "create_group", "username": username, "group": group}
            )
            client.send(payload.encode())
            ...
            payload = json.dumps(
                {"command": "ping", "username": username, "sent_at": time.time()}
            )
            client.send(payload.encode())
